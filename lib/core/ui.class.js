...
class UI {
  ...
  ask(question, defaults) {
    defaults = defaults || '';
    const message = (defaults !== '') ? chalk.yellow(question) + chalk.dim(" (" + defaults + ")") + chalk.white(": ") : chalk.yellow(question) + chalk.white(": ");
  ...
  askBool(question, defaults) {
    ...
    const message = chalk.yellow(question) + " " + ((defaults) ? chalk.dim("(Y/n)") : chalk.dim("(y/N)")) + chalk.white(": ");
