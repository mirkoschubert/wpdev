...
class SSH {
  ...
  async exec(command, dir) {
    const install_dir = dir || false;
    const response = await this.ssh.execCommand(command, (install_dir) ? { cwd: install_dir } : {});
  ...
  async checkVersions() {
    ...
    versions.php = (!php.stderr) ? php.stdout.match(/^PHP\ ([0-9\.]*)/)[1] : php.stderr;
    versions.wpcli = (!wpcli.stderr) ? wpcli.stdout.substr(7) : wpcli.stderr;
    versions.node = (!node.stderr) ? node.stdout.substr(1) : node.stderr;
